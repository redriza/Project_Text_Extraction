...
def remove_footer_noise(text):
    return re.sub(r'(rot.*|॥.*|Vs\|.*|\d+\|\d+\|\d+.*)', '', text, flags=re.IGNORECASE).strip()
...
def extract_verses(text):
    ...
    while i < len(lines):
        ...
        if re.match(r"^\d\.\d+\.\d+", line):
            ...
            translation = remove_footer_noise(translation)
...
def write_csv(rows, out_path):
    ...
    with open(out_path, "w", encoding="utf-8", newline="") as f:
        ...
        last_subchapter = ""
        for row in rows:
            key = (row["sutra_no"], row["sb_verse_roman"])
            if key in seen:
                continue
            seen.add(key)
...
# === MAIN ===
def main():
    print("Converting first 5 pages of PDF to images...")
